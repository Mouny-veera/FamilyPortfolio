...
- **Scanner Universe**: Nifty 500 auto-fetched from NSE (`niftyindices.com` CSV endpoint) on startup and before each scan. Cached in `data/nifty500.json` as fallback (falls back to legacy `nifty200.json`). Service: `services/nifty_index.py`.
