...
- **Market Data**: Fyers API (primary, real-time) with automated headless token refresh; yfinance (fallback). Provider pattern in `services/market_data.py`.
- **Ticker Search**: Fyers public symbols master (`NSE_CM.csv`), parsed on startup into in-memory cache.
...
- **Provider pattern**: `MarketDataProvider` ABC with `FyersProvider` and `YFinanceProvider`. `get_active_provider()` reads `data/config.json` and returns Fyers if configured, else yfinance.
...
- **Fyers auto-login**: On startup, `ensure_valid_token()` checks token validity and refreshes via headless login (TOTP + PIN) if expired. Credentials stored in `data/config.json`.
...
- **NSE tickers**: Store WITHOUT suffix in DB/UI. Add `.NS` for yfinance calls, `NSE:{TICKER}-EQ` for Fyers calls.
